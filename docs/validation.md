...
- Зависимости есть, если есть воркеры, от них зависящие
- Ввод-вывод присутствует и каждый инпорт и аутпорт имеет валидный тип
- Если объявлены воркеры, то каждый воркер ссылается на существующую зависимость
- Сеть присутствует и является валидной

Ввод-вывод модуля валиден когда валидно объявление каждого его порта - каждый порт соотносится с валидным типом.
Тип объявлен корректно в одном из двух случаев:
