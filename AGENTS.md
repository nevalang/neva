...
11. Keep unit tests and benchmarks in separate files (`*_test.go` vs `*_bench_test.go`).
12. Keep this file updated when process/architecture/rules change.
13. For repository-local skills, prefer concise English `SKILL.md` guidance (tool list + workflow); avoid bundled scripts unless explicitly requested.
14. Error semantics policy: return `*compiler.Error` for invalid user programs (input/domain failures), but `panic` on internal invariant violations or impossible cross-stage states.
15. `AGENTS.md` is an engineering harness artifact for both humans and machines.
16. Keep this root file compact; target <=200 lines and move local details into nested `AGENTS.md` files.
17. Treat nested `AGENTS.md` files as the repository source of truth for scoped instructions outside file-type authoring rules.
18. File-type authoring rules live in `.claude/rules/*.md`; if a harness does not load them automatically, inspect matching `paths` frontmatter before editing files.
19. Avoid duplicating durable guidance across `AGENTS.md`, `.claude/rules`, and docs; keep one source of truth and make the other layers point to it.
