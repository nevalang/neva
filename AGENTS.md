...
- **Language semantics**: Union tag-only matching should only succeed when the pattern has no data; tag-only messages should not match data-bearing patterns.
- **Common patterns**: When unboxing unions, return the original union message if it has no data to avoid propagating invalid messages.
- **Common patterns**: Use list/dict-specific equality helpers since Msg values are not comparable with `maps.Equal`.
