...
* **Runtime as dependency in go.mod:** Change default behaviour of the codegen so that runtime is not copyied into the user's go module but instead user is asked to `go get github.com/nevalang/neva@<VERSION>` so that `github.com/nevalang/neva@<VERSION>/pkg/runtime` imports inside generated go code can be resolved. As you can see in this mode imports should be reference to "github.com/nevalang/neva", and `pkg.Version` must be respected. This is how protobuf compiler work.

### Runtime as dependency in go.mod

- If we move runtime to pkg in order to expose it for `go get` we also should move `embed.go` to `pkg`.
- Make sure normal (not library mode) executables do not contain references to third-party modules. Remember - nevalang generated go modules MUST (!!!) always be self-contained and have zero deps!
