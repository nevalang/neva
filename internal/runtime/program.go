...
func (s SingleInport) Receive(ctx context.Context) (OrderedMsg, bool) {
	...
	portTracer(ctx, s.tracer).RecordReceived(slotAddr, ordered)
...
func (a *ArrayInport) Receive(ctx context.Context, idx int) (OrderedMsg, bool) {
	...
	case v := <-a.chans[idx]: //nolint:varnamelen // TODO(strict-lint phase 1): temporary suppression; remove after strict cleanup.
		...
		portTracer(ctx, a.tracer).RecordReceived(slotAddr, v)
	...
	for idx := range a.chans {
		wg.Go(func() {
			...
			case received := <-a.chans[idx]:
				...
				portTracer(ctx, a.tracer).RecordReceived(slotAddr, received)
	...
	for i := 0; len(buf) == 0 || i < len(a.chans); i++ {
		...
		for slotIdx, ch := range a.chans {
			...
			case orderedMsg := <-ch:
				...
				portTracer(ctx, a.tracer).RecordReceived(slotAddr, orderedMsg)
...
func (s SingleOutport) Send(ctx context.Context, msg Msg, causes ...OrderedMsg) bool {
	...
	hop := portTracer(ctx, s.tracer).RecordSent(slotAddr, ordered, causes)
...
func (a *ArrayOutport) Send(ctx context.Context, idx uint8, msg Msg, causes ...OrderedMsg) bool {
	...
	hop := portTracer(ctx, a.tracer).RecordSent(slotAddr, ordered, causes)
...
func portTracer(ctx context.Context, tracer *Tracer) *Tracer {
	if tracer != nil {
		return tracer
	}
	fromCtx, ok := tracerFromContext(ctx)
	if !ok {
		return NewTracer()
	}
	return fromCtx
