...
type compatCheckerTestcase struct {
	wantErr        error
	scope          TestScope
...
var compatCheckerTests = []compatCheckerTestcase{
	//  kinds
	{
	...
	{ // union {Int} <: union {Int int, Str string}
		name: "tag-only union selector is compatible with payload union tag",
		subType: h.Union(map[string]*ts.Expr{
			"Int": nil,
		}),
		superType: h.Union(map[string]*ts.Expr{
...
func TestCompatChecker_Check(t *testing.T) {
	t.Parallel()
	for _, tt := range compatCheckerTests {
